...
const DepthCarousel = ({
  ...
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!shown) opacity = 0;
      if (cfg.compact && az > 0.05) opacity = 0;
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      const cfg = cfgRef.current;
      if (cfg.compact) {
        scaleRef.current = Math.min(1, w / (cfg.cardWidth + 16));
      } else {
  ...
  return (
    <div
      ref={rootRef}
      className={`relative flex h-full w-full cursor-grab touch-pan-y select-none items-center justify-center outline-none [perspective-origin:50%_50%] active:cursor-grabbing focus-visible:rounded-xl focus-visible:outline-2 focus-visible:outline-white/50 focus-visible:[outline-offset:4px] ${
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className="absolute left-1/2 top-1/2 cursor-pointer overflow-hidden border border-white/[0.06] bg-transparent shadow-[0_16px_40px_-20px_rgba(0,0,0,0.55)] [transform:translate(-50%,-50%)] [transform-origin:center] [will-change:transform,opacity,filter]"
            ref={el => {
          ...
          >
            <img
              className="block h-full w-full select-none object-cover object-center [pointer-events:none] [-webkit-user-drag:none]"
              src={item.image}
              alt={item.alt || ''}
              draggable={false}
            />
      ...
      {showControls && count > 1 && (
        <>
          <button
            type="button"
            className={`absolute top-1/2 z-[3000] grid -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-[rgba(18,20,26,0.55)] text-white backdrop-blur-md transition-[background,border-color,transform] duration-200 hover:border-white/40 hover:bg-[rgba(28,31,40,0.85)] active:scale-95 ${
              compact ? "left-2 h-9 w-9" : "left-4 h-[42px] w-[42px]"
            }`}
            aria-label="Previous slide"
            onClick={() => navigateBy(-1)}
          >
            <svg viewBox="0 0 24 24" width={compact ? 16 : 20} height={compact ? 16 : 20} aria-hidden="true">
              <path
                d="M15 5l-7 7 7 7"
                fill="none"
                stroke="currentColor"
          ...
          <button
            type="button"
            className={`absolute top-1/2 z-[3000] grid -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-[rgba(18,20,26,0.55)] text-white backdrop-blur-md transition-[background,border-color,transform] duration-200 hover:border-white/40 hover:bg-[rgba(28,31,40,0.85)] active:scale-95 ${
              compact ? "right-2 h-9 w-9" : "right-4 h-[42px] w-[42px]"
            }`}
      ...
      {showIndicators && count > 1 && (
        <div
          className={`absolute left-1/2 z-[3000] flex -translate-x-1/2 gap-2 rounded-full bg-[rgba(14,16,22,0.4)] backdrop-blur-sm ${
